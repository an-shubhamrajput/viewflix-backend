...
def build_movie_feature_matrix(
    ...
    pop_norm = PopularityNormalizer.from_values(
        [m.popularity_raw for m in movies]
    )
...
def build_full_feature_matrix_for_user(
    ...
    user_vec = np.tile(user_vec.reshape(1, -1), (len(movies), 1))
    ...
    match_arr = np.asarray(match_scores, dtype=np.float32).reshape(-1, 1)
